...
class MovieAddRatingForm(Form):
    ...
    choices_genre = [('Choose selection:', 'Choose selection:'),
                     ...
                     ('International', 'International'), ('Romance', 'Romance'), ('Sci-Fi', 'Sci-Fi'),
                     ('Sport', 'Sport'), ]
    ...
    if request.method == "POST":
        ...
        if search.data['add_table'] == 'User Rating':
            ...
            cursor.execute("""SELECT * FROM Recommendations WHERE MovieID IN(SELECT MovieID
                                                                           FROM Movies
                                                                           WHERE Title = %s AND DeletedAt IS NULL);""",
                           (title,))
            result = cursor.fetchall()
            ...
            cursor.execute("""SELECT *  FROM Directors WHERE DirectorName = %s AND DeletedAt IS NULL;""",
                           (director_name,))
            result = cursor.fetchall()
            if cursor.rowcount != 0:
...
def update():
    ...
    if form.validate_on_submit():
        ...
        if cursor.rowcount == 0:
            ...
            cursor.execute(
                """UPDATE Movies SET Category = %s, Title = %s, Country = %s, ReleaseYear = %s, Duration = %s, Description = %s WHERE Title = %s;""",
                (category, title, country, release_year, duration, description, title,))
            cursor.execute(
                """UPDATE Genres LEFT JOIN Movies ON(Movies.MovieId = Genres.MovieId) SET Genres.GenreType = %s WHERE Movies.Title = %s;""",
                (genre, title,))
            cursor.execute(
                """UPDATE Ratings LEFT JOIN Movies ON(Movies.MovieId = Ratings.MovieId) SET Ratings.Rating = %s WHERE Movies.Title = %s;""",
                (tv_rating, title,))
            cursor.execute(
                """UPDATE Recommendations LEFT JOIN Movies ON(Movies.MovieId = Recommendations.MovieId) SET Recommendations.UserRating = %s WHERE Movies.Title = %s;""",
                (user_rating, title,))
            cursor.execute(
                """UPDATE StreamingService LEFT JOIN Movies ON(Movies.MovieId = StreamingService.MovieId) SET StreamingService.Platform = %s, DateAdded = current_date WHERE Movies.Title = %s;""",
                (platform, title,))
            conn.commit()

            cursor.execute(
                """SELECT Movies.*, GenreType, Rating, Platform, DateAdded, UserRating FROM Movies LEFT JOIN Genres ON (Movies.MovieId = Genres.MovieId) LEFT JOIN Ratings ON (Movies.MovieId = Ratings.MovieId) LEFT JOIN StreamingService ON (StreamingService.MovieId= Movies.MovieId) LEFT JOIN Recommendations ON (Recommendations.MovieId= Movies.MovieId) WHERE Title = %s;""",
                (title,))
            result = cursor.fetchall()
        ...
        if request.method == "POST":
            ...
            if not form.countries_enter.data:
                all_countries_list = ['Argentina', 'Australia', 'Austria', 'Brazil', 'Bulgaria', 'Canada',
                                      ...
                                      'Philippines', 'Romania', 'South Korea', 'Spain', 'Sweden', 'Switzerland',
                                      'Tanzania', 'Thailand',
                                      'Turkey', 'United Kingdom', 'UK', 'United States', 'USA', 'Vietnam']
            ...
            if not form.genre_enter.data:
                all_genre_list = ['Action', 'Adventure', 'Animation', 'Comedies', 'Crime', 'Documentaries', 'Dramas',
                                  'Family', 'Horror', 'Independent', 'Romantic', 'Sci-Fi', 'Sports']
            ...
            if request.form['search_options'] == 'Title':
                ...
                cursor.execute(
                    """SELECT Movies.*, GenreType, Rating, Platform, DateAdded, UserRating FROM Movies LEFT JOIN Genres ON (Movies.MovieId = Genres.MovieId) LEFT JOIN Ratings ON (Movies.MovieId = Ratings.MovieId) LEFT JOIN StreamingService ON (StreamingService.MovieId= Movies.MovieId) LEFT JOIN Recommendations ON (Recommendations.MovieId= Movies.MovieId) WHERE Title LIKE %s AND Movies.DeletedAt IS NULL ORDER BY Title;""",
                    (search_string,))
                result = cursor.fetchall()
                if cursor.rowcount == 0:
                    flash('No results found!')
                ...
                search_string = "%" + request.form['search_field'] + "%"
                cursor.execute("""SELECT * FROM Actors WHERE ActorName LIKE %s AND DeletedAt IS NULL;""",
                               (search_string,))
                result = cursor.fetchall()
                if cursor.rowcount == 0:
                    flash('No results found!')
                ...
                search_string = "%" + request.form['search_field'] + "%"
                cursor.execute("""SELECT * FROM Directors WHERE DirectorName LIKE %s AND DeletedAt IS NULL;""",
                               (search_string,))
                result = cursor.fetchall()
                if cursor.rowcount == 0:
                    flash('No results found!')
                ...
                search_string = request.form['search_field']
                cursor.execute(
                    """SELECT Movies.*, GenreType, Rating, Platform, DateAdded, UserRating FROM Movies LEFT JOIN Genres ON (Movies.MovieId = Genres.MovieId) LEFT JOIN Ratings ON (Movies.MovieId = Ratings.MovieId) LEFT JOIN StreamingService ON (StreamingService.MovieId= Movies.MovieId) LEFT JOIN Recommendations ON (Recommendations.MovieId= Movies.MovieId) WHERE UserRating = %s AND Movies.DeletedAt IS NULL ORDER BY Title;""",
                    (search_string,))
                result = cursor.fetchall()
                if cursor.rowcount == 0:
                    flash('No results found!')
                ...
                cursor.execute(
                    "SELECT Movies.*, GenreType, Rating, Platform, DateAdded, UserRating FROM Movies LEFT JOIN Genres ON (Movies.MovieId = Genres.MovieId) LEFT JOIN Ratings ON (Movies.MovieId = Ratings.MovieId) LEFT JOIN StreamingService ON (StreamingService.MovieId= Movies.MovieId) LEFT JOIN Recommendations ON (Recommendations.MovieId= Movies.MovieId) WHERE Movies.DeletedAt IS NULL AND Category IN ({}) AND Platform IN ({}) AND Rating IN ({}) AND "
                    ...
                    "OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s OR Country LIKE %s) ORDER BY Title".format(
                        str(categories)[1:-1], str(platforms)[1:-1], str(entered_ratings)[1:-1]), (
                    genre_list[0], genre_list[1], genre_list[2], genre_list[3], genre_list[4], genre_list[5],
                    genre_list[6], genre_list[7], genre_list[8], genre_list[9], genre_list[10],
                    genre_list[11], genre_list[12], countries_list[0], countries_list[1], countries_list[2],
                    countries_list[3], countries_list[4],
                    ...
                    countries_list[15], countries_list[16], countries_list[17], countries_list[18], countries_list[19],
                    countries_list[20],
                    countries_list[21], countries_list[22], countries_list[23], countries_list[24], countries_list[25],
                    countries_list[26],
                    countries_list[27], countries_list[28], countries_list[29], countries_list[30], countries_list[31],
                    countries_list[32],
                    countries_list[33], countries_list[34], countries_list[35], countries_list[36], countries_list[37],
                    countries_list[38],
                    countries_list[39], countries_list[40], countries_list[41], countries_list[42],))
...
def suggestions():
    ...
    cursor.execute(
        "SELECT GenreType, MAX(UserRating) FROM Genres INNER JOIN Recommendations ON (Recommendations.MovieId= Genres.MovieId) GROUP BY GenreType ORDER BY MAX(UserRating) DESC LIMIT 5")
    ...
    # print(new_list[0])
    cursor.execute(
        "SELECT Movies.*, GenreType, Rating, Platform, DateAdded, UserRating FROM Ratings INNER JOIN Movies ON (Movies.MovieId= Ratings.MovieId) LEFT JOIN Genres ON (Movies.MovieId = Genres.MovieId) LEFT JOIN StreamingService ON (StreamingService.MovieId= Movies.MovieId)LEFT JOIN Recommendations ON (Recommendations.MovieId= Movies.MovieId) WHERE Movies.DeletedAt IS NULL AND Rating IN ({}) AND (GenreType LIKE %s OR GenreType LIKE %s  OR GenreType LIKE %s  OR GenreType LIKE %s OR GenreType LIKE %s) AND UserRating IS NULL LIMIT 10".format(
            str(list_ratings)[1:-1]), (new_list[0], new_list[1], new_list[2], new_list[3], new_list[4],))
    result = cursor.fetchall()
